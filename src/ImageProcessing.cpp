...
void imageProcessingFun(const QString& progName, QImage* const outImgs, const QImage* const inImgs, const QVector<double>& params) 
{
	...
	uchar* Y_buff_helper = new uchar[inImgs->width()*inImgs->height()];
	...
	*outImgs = *(new QImage(X_SIZE, Y_SIZE, inImgs->format()));
	...
	else if (progName == "DCT Quantization") 
	{
		...
		/* TODO */		
		for (int i = 0; i < inImgs->width()*inImgs->height(); i++) {
			Y_buff_helper[i] = Y_buff[i];
		}
		...
		for (int i = 0; i < inImgs->width()*inImgs->height(); i++) {
			MSE += (Y_buff_helper[i] - Y_buff[i])*(Y_buff_helper[i] - Y_buff[i]);
		}
		
		MSE /= inImgs->width()*inImgs->height();
	}
